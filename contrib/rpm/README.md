...
The RPM spec file provided here is for Groincoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://groincoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Groincoin git repository](https://github.com/groincoin/groincoin) but are not part of
...
Currently only `Source10` is used, for the example `groincoin.conf` file.
...
in the groincoin git repository. At present only an SVG version of the Groincoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the groincoin
...
needed when 0.12.1 is released, a proper fix is already in the Groincoin git
master branch.
...
idea to build Groincoin Core against a static version of that library compiled
...
The `groincoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `groincoin-server` package. At this point, most RPM based
...
The `groincoin-server` package should have SELinux support. How to properly do
...
Groincoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/groincoin/). He has been
packaging Groincoin for Fedora at least since 2012.
...
4. I renamed the `groincoin` package that contains the Qt GUI to `groincoin-core` as
that appears to be how the general population refers to it, in contrast to
`groincoin-xt` or `groincoin-classic`. I wanted to make sure the general population
