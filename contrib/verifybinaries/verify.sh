...
###   groincoincore.org and groincoin.org and compares them.
...
WORKINGDIR="/tmp/groincoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://groincoincore.org"
HOST2="https://groincoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="groincoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'groincoin-',
...
if [ $? -ne 0 ]; then
   echo "groincoin.org failed to provide signature file, but groincoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "groincoin.org and groincoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Groincoin Core binary release signing key installed?"
   fi
