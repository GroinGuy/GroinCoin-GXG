#!/bin/bash
# Copyright (c) 2013 The Groincoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Groincoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Groincoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Groincoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Groincoin 0.7 datadir."
    ;;
  3)
    echo "Detected Groincoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Groincoin 0.8 datadir."
    ;;
esac
