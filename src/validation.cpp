...
#if defined(NDEBUG)
# error "Groincoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Groincoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GXG additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("groincoin-scriptch");
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    //printf("blockhash = %s nbits = %x BlockHeight= %d \n", block.GetHash().ToString().c_str(), block.nBits, nHeight);
	if(nHeight >= 60){
		 uint32_t nBitsNext = GetNextWorkRequired(pindexPrev, &block, consensusParams);
