 ...
 * between groincoind, and groincoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("groincoin-shutoff");
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), GROINCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/GroinGuy/GroinCoin-GXG>";
    const std::string URL_WEBSITE = "<http://groincoin.com/>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("groincoin-loadblk");
...
/** Sanity checks
 *  Ensure that Groincoin is running in a usable environment with all
...
{
    // Make sure only a single Groincoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", GROINCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if groincoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if groincoin is started while in a temporary directory.\n",
